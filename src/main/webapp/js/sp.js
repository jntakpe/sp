var spApp = angular.module('spApp', ['http-auth-interceptor', 'ngResource', 'ngRoute']);
...
spApp.config(['$routeProvider',
    ...
    }]).run(['$rootScope', '$location', 'SPAuthService', 'UserResource',
    function ($rootScope, $location, SPAuthService, UserResource) {
        "use strict";

        //Fonction utilisée lors d'un changement d'url pour vérifier si l'utilisateur est authentifié.
        $rootScope.$on("$routeChangeStart", function (event, next, current) {
            SPAuthService.authenticate(function () {$rootScope.authOK = true;});
        });
